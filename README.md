Day1 practice session for FSD Training.

=> primarly connected the gitHub account to the Git Bash
=> used git commands:
    -config (for username and email)
    -init = initializing the empty repository at a path (since it is the new repo)
    -status = displayed all the tracked and untracked files with the status and commits done in the repo.
    -add "filename" = created a file and added to  the folder.
